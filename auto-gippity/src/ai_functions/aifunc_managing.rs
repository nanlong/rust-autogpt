...
pub fn convert_user_input_to_goal(_user_request: &str) {
    ...
    /// Example 2:
    ///     user_request = "Create something that stores crypto price data in a database using supabase and retrieves prices on demand"
    ///     OUTPUT: "build a website that fetches and stores crypto price data within a supabase setup including a fronted UI"
    println!(OUTPUT)
}
